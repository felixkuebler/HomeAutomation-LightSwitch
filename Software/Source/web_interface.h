...
String generateMQTTSettings(wifi_obj wifi_info, io_list input_list, io_list output_list)
{
  String page = "";
  ...
  page += "<div class=\"hidden\" id=\"mqtt\">";
    ...
    page += "<br>";
    page += "<div>";
      page += "Broker(Server) IP ";
      page += "<input class=\"text-input\" type=\"text\" id=\"mqtt_broker\" value=\"" + wifi_info.mqtt_broker + "\">";
    page += "</div>";
    page += "<br>";
    page +="<div class=\"info\">Enter the local IP of your MQTT Broker/Server (e.g. Mosquitto on a Raspbarry Pi).</div>";
    page += "<br>";
    page += "<div>";
    page += "<br>";


      //loop will apply input tabs
      short int i=0;
      for (io_list::iterator it=input_list.begin(); it!=input_list.end(); ++it)
      {
        if(i==0)
        {
          page += "<button class=\"btn btn-options selected\" onclick=\"handleOptions(this)\" for=\"" + it->first + "\">" + it->first + "</button>";
        }
        else
        {
          page += "<button class=\"btn btn-options\" onclick=\"handleOptions(this)\" for=\"" + it->first + "\">" + it->first + "</button>";
        }
        i++;
      }

      //loop will apply output tabs
      ...
      //loop will apply input options
      i=0;
      for (io_list::iterator it=input_list.begin(); it!=input_list.end(); ++it)
      {
        if(i==0)
        {
          page += "<div class=\"sub-frame\" id=\"" + it->first + "\">";
        }
        else
        {
          page += "<div class=\"hidden sub-frame\" id=\"" + it->first + "\">";
        }
        i++;
          page += "<div>";
            page += "Topic ";
            page += "<input class=\"text-input\" type=\"text\" id=\"" + it->first + "_topic\" value=\"" + it->second.topic + "\">";
          page += "</div>";
          page += "<br>";
          page +="<div class=\"info\">Define the topic to adress the following commands to a group of devices that use the same topic name.</div>";
          page += "<br>";

          page += "<div style=\"width: 30vw; display: table; margin: 0 auto\">";
            page += "<div style=\"display: table-row\">";
            
              page += "<div style=\"display: table-cell;\">Commands </div>";
              
              page += "<div style=\"display: table-cell;\">";
                page += "<div>";
                  page += "on/off ";
                  if(it->second.on_command.length() > 0 && it->second.off_command.length() > 0)
                  {
                    page += "<input type=\"radio\" name=\"" + it->first + "\" value=\"onoff\" checked=\"checked\">";
                  }
                  else 
                  {
                    page += "<input type=\"radio\" name=\"" + it->first + "\" value=\"onoff\">";
                  }
                page += "</div>";
                page += "<div>";
                  page += "toggle ";
                  ...
                  {
                    page += "<input type=\"radio\" name=\"" + it->first + "\" value=\"toggle\" checked=\"checked\">";
                  }
                  else 
                  {
                    page += "<input type=\"radio\" name=\"" + it->first + "\" value=\"toggle\">";
                  }
                page += "</div>";
              page += "</div>";
          ...
          page += "<br>";
          page +="<div class=\"info\">";
            page +="Specify the type sof commands this node is going to use. \"on/off\" enables sends/receive of \"on\" and \"off\" commands depending on the state of the switch.";
            page +=" \"toggle\" will enable receiving/sending of \"toggle\"commands. A sender (switches) can only send either \"on/off\" or \"toggle\" while a receiver (relay) can also accept both.";
          page +="</div>";
        page += "</div>";
      }

      //loop will apply output options
      for (io_list::iterator it=output_list.begin(); it!=output_list.end(); ++it)
      {
        if(i==0)
        {
          ...
          page += "<div style=\"width: 30vw; display: table; margin: 0 auto\">";
            page += "<div style=\"display: table-row\">";
              ...
              page += "<div style=\"display: table-cell;\">";             
                page += "<div>";
                  page += "on/off ";
                  if((it->second.on_command.length() > 0) && (it->second.off_command.length() > 0) && (it->second.toggle_command.length() <= 0))
                  {
                    page += "<input type=\"radio\" name=\"" + it->first + "\" value=\"onoff\" checked=\"checked\">";
                  }
                  else 
                ...
                page += "</div>";
                page += "<div>";
                  page += "on/off and toggle ";
                  if((it->second.on_command.length() > 0) && (it->second.off_command.length()) > 0 && (it->second.toggle_command.length() > 0))
                  {
                    page += "<input type=\"radio\" name=\"" + it->first + "\" value=\"onofftoggle\" checked=\"checked\">";
                  }
                  else 
                  {
                    page += "<input type=\"radio\" name=\"" + it->first + "\" value=\"onofftoggle\">";
                  }          
                page += "</div>";
              page += "</div>"; 
            page += "</div>";
  ...
  script += "<script>";
    ...
    script += "function sendSettings(){";
      ...
      script += "for (var i = 0; i < inputs.length; ++i){";
        ...
        script += "if(!value) value=\"-1\";";
        script += "var type = inputs[i].getAttribute(\"type\");";
        script += "if(type == \"radio\"){";
          script += "if(inputs[i].checked){";
            script += "var name = inputs[i].getAttribute(\"name\");";
            script += "if(value == \"onoff\"){";
              script += "msg+= name +\"_on_command=on&\" ;";
              script += "msg+= name +\"_off_command=off&\" ;";
              script += "msg+= name +\"_toggle_command=-1\" ;";
            script += "}";
            script += "else if(value == \"toggle\"){";
              script += "msg+= name +\"_on_command=-1&\" ;";
              script += "msg+= name +\"_off_command=-1&\" ;";
              script += "msg+= name +\"_toggle_command=toggle\" ;";
            script += "}";
            script += "else if(value == \"onofftoggle\"){";
              script += "msg+= name +\"_on_command=on&\" ;";
              script += "msg+= name +\"_off_command=off&\" ;";
              script += "msg+= name +\"_toggle_command=toggle\" ;";
            script += "}";
...
String generateWebpage(wifi_obj wifi_info, io_list input_list, io_list output_list)
{
  String page = "";
    ...
    page += "<body>";
      ...
      page += "<div class=\"main-frame\">";
        ...
        page += "<div class=\"right-frame\">";
          ...
          page += generateMQTTSettings(wifi_info, input_list, output_list);
