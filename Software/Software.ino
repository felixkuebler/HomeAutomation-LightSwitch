...
{  
  ...
  {   
    ...
    {
      buff[i++] = (long int)((unsigned char)Serial.read());
      delay(10);
    }

    Serial.print(i);
    Serial.print(" digits available \n");


    for(int j=0; j< i; j++)
    {
      Serial.print(buff[j]);   // read it and send it out Serial1 (pins 0 & 1)
      if(j+1>= i) break;
      Serial.print('-');   // read it and send it out Serial1 (pins 0 & 1)
    }
    Serial.print("\n");

    rfid_connection=true;
  }
}
